...
//FLYME:safe@System.Security.Feature.Permission {@
bool CameraService::isCameraPrivacyWhiteList(const String16& packageName, int callingUid) {
    ALOGI("check isCameraPrivacyWhiteList,callingUid %d", callingUid);
    if (callingUid == AID_SYSTEM){
        ALOGI("isCameraPrivacyWhiteList, AID_SYSTEM");
        return true;
    }
    std::vector <String16> allowedPackages = {
            toString16("android"), // 系统面容
            toString16("com.meizu.facerecognition"), // 面容识别
            toString16("com.meizu.voiceassistant"), //语音后台视频连麦
            toString16("com.meizu.flyme.service.find"), //查找手机远程拍照
            toString16("android.sensorprivacy.cts.usemiccamera"),
            toString16("com.android.camera2")
    };

    for (const auto& allowedPackage : allowedPackages) {
        if (packageName == allowedPackage) {
            ALOGI("isCameraPrivacyWhiteList true");
            return true;
        }
    }
    return false;
...
Status CameraService::connectHelper(const sp<CALLBACK>& cameraCb, const std::string& cameraId,
    ...
    {
        ALOGI("CameraService::connectHelper called, cameraId %s",  cameraId.c_str());
        ...
        //FLYME:safe@System.Security.Feature.Permission {@
        isCameraPrivacyEnabled = isCameraPrivacyEnabled
                && !this ->isCameraPrivacyWhiteList(toString16(client->getPackageName()),clientAttribution.uid);
        // @}
...
status_t CameraService::BasicClient::handlePermissionResult(
    ...
    if (result == PermissionChecker::PERMISSION_HARD_DENIED) {
        ...
        //FLYME:safe@System.Security.Feature.Permission {@
        if (sCameraService ->isCameraPrivacyWhiteList(toString16(getPackageName()),getClientUid())){
            return OK;
        }
        // @}
...
void CameraService::BasicClient::opChanged(int32_t op, const String16&) {
    ...
    if (res == PermissionChecker::PERMISSION_HARD_DENIED) {
        ...
        //FLYME:safe@System.Security.Feature.Permission {@
        isCameraPrivacyEnabled = isCameraPrivacyEnabled
                                 && !sCameraService ->isCameraPrivacyWhiteList(toString16(getPackageName()),getClientUid());
        // @}
