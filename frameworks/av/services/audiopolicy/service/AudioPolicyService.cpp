...
bool AudioPolicyService::SensorPrivacyPolicy::isAudioPrivacyWhiteList(const String16& packageName, int callingUid) {
    ALOGI("check isAudioPrivacyWhiteList,callingUid %d", callingUid);
    if (callingUid == AID_SYSTEM){
        ALOGI("isAudioPrivacyWhiteList, AID_SYSTEM");
        return true;
    }
    std::vector <String16> allowedPackages = {
            String16("android"), // 系统面容
            String16("com.meizu.facerecognition"), // 面容识别
            String16("com.meizu.voiceassistant"), //语音后台视频连麦
            String16("com.meizu.flyme.service.find"), //查找手机远程拍照
            String16("android.sensorprivacy.cts.usemiccamera"),
            String16("com.flyme.auto.permission.whitelist")
    };

    for (const auto& allowedPackage : allowedPackages) {
        if (packageName == allowedPackage) {
            ALOGI("isAudioPrivacyWhiteList true");
            return true;
        }
    }
    return false;
...
bool AudioPolicyService::SensorPrivacyPolicy::isSensorPrivacyEnabled() {
    ALOGI("isSensorPrivacyEnabled false");
    return false;
